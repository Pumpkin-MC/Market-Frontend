...
export const SecuritySetupCards: React.FC<SecuritySetupCardsProps> = ({
    ...
    const toggleBox = (box: 'passkey' | 'totp') => {
        if (expandedBox === box) {
            ...
            setExpandedBox(box);
            if (box === 'totp' && !totpQr && !totpSuccess) {
                loadTotpSetup();
            }
        }
    };
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            ...
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.2rem' }}>
                                    <span style={{ fontWeight: 700, fontSize: '1.05rem', color: 'var(--mp-text, #f8fafc)' }}>
                                        Passkey & Biometrics
                                    </span>
                                    <span style={{
                                        fontSize: '0.72rem',
                                ...
                                <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--mp-muted, #94a3b8)' }}>
                                    Fastest & most secure. Touch ID, Face ID, Windows Hello, or Security Key.
                                </p>
                            </div>
                        </div>
                        <div style={{ color: 'var(--mp-muted, #94a3b8)', marginLeft: '1rem' }}>
                            {expandedBox === 'passkey' ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                        </div>
                    </button>
                        ...
                        <div style={{
                            ...
                            {passkeySuccess ? (
                                <div style={{
                                    ...
                                    <Check size={18} />
                                    <span>Passkey created successfully! Redirecting...</span>
                                </div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                                    <div>
                                        <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: 'var(--mp-text, #f8fafc)', marginBottom: '0.4rem' }}>
                                    ...
                                    >
                                        {passkeyLoading ? (
                                            <>
                                                <Loader2 size={18} className="animate-spin" /> Setting up Passkey...
                                            </>
                                        ) : (
                                            <>
                                                <Fingerprint size={18} /> Create & Register Passkey
                                            </>
                                        )}
                                    </button>
                                </div>
                ...
                <div style={{
                    ...
                    >
                        ...
                        <div style={{ color: 'var(--mp-muted, #94a3b8)', marginLeft: '1rem' }}>
                            {expandedBox === 'totp' ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                        </div>
                    </button>
                    ...
                    {expandedBox === 'totp' && (
                        <div style={{
                            ...
                            {totpLoading ? (
                                <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--mp-muted, #94a3b8)' }}>
                                    <Loader2 size={24} className="animate-spin" style={{ margin: '0 auto 0.5rem auto' }} />
                                    <p style={{ margin: 0 }}>Generating 2FA Secret & QR Code...</p>
                                </div>
                            ) : totpSuccess ? (
                                <div style={{
                                    display: 'flex',
                                    ...
                                    <Check size={18} />
                                    <span>Two-Factor Authentication enabled successfully! Redirecting...</span>
                                </div>
                            ) : (
                                <div>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem', alignItems: 'center', marginBottom: '1.5rem' }}>
                                        {totpQr && (
                                            <div style={{
                                                ...
                                                <img src={totpQr} alt="2FA QR Code" style={{ width: 140, height: 140, display: 'block' }} />
                                            </div>
                                        )}
                                        <div style={{ flex: 1, minWidth: 200 }}>
                                            <p style={{ margin: '0 0 0.5rem 0', fontSize: '0.85rem', color: 'var(--mp-text, #f8fafc)', fontWeight: 600 }}>
                                                1. Scan this QR Code with your Authenticator App
                                            </p>
                                            <p style={{ margin: '0 0 0.8rem 0', fontSize: '0.8rem', color: 'var(--mp-muted, #94a3b8)' }}>
                                                Or manually copy the setup key below if you cannot scan the code:
                                            </p>
                                            {totpSecret && (
                                                <div style={{
                                                    display: 'flex',
                                                    ...
                                                    <code style={{ fontSize: '0.85rem', color: '#f97316', flex: 1, letterSpacing: '0.05em' }}>
                                                        {totpSecret}
                                                    </code>
                                                    <button
                                                        type="button"
                                    ...
                                    <form onSubmit={handleVerifyTotp} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                                        <div>
                                            <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: 'var(--mp-text, #f8fafc)', marginBottom: '0.4rem' }}>
                                        ...
                                        >
                                            {totpVerifying ? (
                                                ...
                                                <>
                                                    <AlertCircle size={18} /> {totpError} — Try Again
                                                </>
                                            ) : (
                                                <>
                                                    <ShieldCheck size={18} /> Verify & Enable 2FA
                                                </>
                                            )}
                                        </button>
                                    </form>
